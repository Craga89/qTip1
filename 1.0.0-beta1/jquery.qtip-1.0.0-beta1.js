...
(function($)
{
   ...
   $.fn.qtip = function(options) 
   {
      ...
      if(typeof options == 'string')
      {
         switch(options)
         {
            case 'api': return $(this).data("qtip");
               break;
               
            case 'disable': return $(this).data("qtip").disable(true);
               break;
               
            case 'enable': return $(this).data("qtip").disable(false);
               break;
               
            case 'destroy': return $(this).data("qtip").destroy();
               break;
      ...
      if(typeof options.style !== 'object') options.style = { name: 'cream' };
      ...
      // Build style options
      opts.style = $.extend(true, {}, $.fn.qtip.styles.defaults, 
         $.fn.qtip.styles[opts.style.name] || $.fn.qtip.styles.cream, opts.style);
      ...
      return this.each(function()
      {
         ...
         // Set target element if not present
      ...
      $.extend(self,
      {
         construct: function()
         {
            ...
            self.elements.tooltip = $(document.createElement('div'))
               .addClass('qtip')
               .addClass(self.options.style.classes.tooltip)
            ...
            self.elements.tooltip.css({ width: self.elements.tooltip.outerWidth() + 6 });
         ...
         getVersion: function(){ return [1, 0, 0, 'beta1']; },
   ...
   function createBorder()
   {
      ...
      else
      {
         ...
         {
            //Adjust content to inline-block (Border bug)
            self.elements.content.css({ display: 'inline-block' });
            ...
            {
               // Create VML arc
               $(document.createElement('v:roundrect'))
                  .attr('fill', 'true')
                  ...
                  .css({ 
                     width: radius * 2, 
                     height: radius * 2, 
                     marginTop: borders[i][1], 
                     marginLeft: borders[i][0]
                  })
                  .appendTo(shapes[i]);
         ...
         var borderTop = $(document.createElement('div'))
            .addClass('borderTop')
            .css({ height: radius, width: self.elements.tooltip.outerWidth() })
         ...
         var borderBottom = $(document.createElement('div'))
            ...
            .css({ height: radius, clear: 'both', width: self.elements.tooltip.outerWidth() })
   ...
   function createTip(corner)
   {
      ...
      else if(corner.search(/left|right/) !== -1)
      {
         ...
         { 
            self.elements.tip.css({ marginTop: radiusAdjust }); 
            self.elements.tooltip.css({ marginTop: -radiusAdjust }) 
         }
         else if(corner.search(/Bottom/) !== -1)
         { 
            self.elements.tip.css({ marginTop: Math.floor(self.elements.tooltip.outerHeight() - self.options.style.tip.size.y) }); 
            self.elements.tooltip.css({ marginTop: radiusAdjust })
         }
   ...
   function assignEvents(state)
   {
      ...
      if(state !== false)
      {
         ...
         // Return if no show event is specified
         if(self.options.show.when.event === false) return;
         ...
         // Hide event is regular DOM event
         ...
         {
            // Define events which reset hide timer
            var resetEvents = [
            ...
            { 
               ...
               self.timers.inactive = setTimeout(function()
               {
                  ...
                  $(resetEvents).each(function(){
                     hideTarget.unbind(this, setTimer);
                     self.elements.tooltip.unbind(this, setTimer);
                  });
            ...
            $(resetEvents).each(function(){
               hideTarget.bind(this, setTimer);
               self.elements.tooltip.bind(this, setTimer);
            });
         }
      }
