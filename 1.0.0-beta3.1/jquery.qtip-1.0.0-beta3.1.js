...
(function($)
{
   ...
   $.fn.qtip = function(options) 
   {
      ...
      if(typeof options == 'object') 
      {
         ...
         // Build style options
         var styleExtend = [ opts.style ];
         while(typeof styleExtend[0].name == 'string')
         {
            styleExtend.unshift( $.fn.qtip.styles[ styleExtend[0].name ] );
         }
         styleExtend.unshift(true, {classes:{ tooltip: 'qtip-'+opts.style.name }}, $.fn.qtip.styles.defaults);
         
         // Extend into a single style object
         opts.style = $.extend.apply($, styleExtend);
      }
   ...
   function assignEvents(state)
   {
      ...
      if(state !== false)
      {
         ...
         {
            if($.isFunction(showMethod)) showTarget.bind(self.options.show.when.event + '.qtip', showMethod);
            if($.isFunction(hideMethod)) hideTarget.bind(self.options.hide.when.event + '.qtip', hideMethod);
         }
      }
